  ...
  return (
    <Wrapper>
      <Form onSubmit={handleSubmit(onSubmit)}>
        {hasErrored && <ErrorMessage>Mail gönderilemedi. Tekrar deneyin.</ErrorMessage>}
        <InputGroup>
          <InputStack>
            {errors.name && <ErrorMessage>İsim girilmesi gerekli.</ErrorMessage>}
            <Input placeholder="Adınız" id="name" disabled={isDisabled} {...register('name', { required: true })} />
          </InputStack>
          <InputStack>
            {errors.email && <ErrorMessage>E-mail girilmesi gerekli</ErrorMessage>}
            <Input placeholder="E-mail" id="email" disabled={isDisabled} {...register('email', { required: true })} />
          </InputStack>
        </InputGroup>
        <InputStack>
          {errors.description && <ErrorMessage>Mesaj girilmesi gerekli.</ErrorMessage>}
          <Textarea
            as="textarea"
